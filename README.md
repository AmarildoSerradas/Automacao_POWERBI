...
Execução contínua e sem intervenção manual
🧠 Como funciona

O script utiliza o Playwright para simular a navegação do usuário, acessando relatórios no Power BI, aplicando filtros e capturando os dados visuais. Em seguida, as imagens são tratadas e enviadas automaticamente para grupos específicos no WhatsApp.

▶️ Como executar
pip install playwright pillow pywin32
playwright install
python nome_do_arquivo.py
⚠️ Pontos de atenção (realidade, sem romantizar)
Dependente de elementos da interface (pode quebrar se o Power BI mudar)
Uso de time.sleep() ao invés de waits inteligentes (não é o ideal)
Código grande e pouco modular (difícil manutenção)
